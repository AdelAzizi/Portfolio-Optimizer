...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('preprocessor.log', encoding='utf-8'),
...
class DataPreprocessor:
    ...
    def download_all_price_data(self, universe: List[str], years_of_data: float, batch_size: int = 50) -> Tuple[pd.DataFrame, List[str]]:
        """Download historical price data for the investment universe in batches."""
        price_data = {}
        ...
        if not price_data:
            logger.warning("⚠️ No price data was successfully downloaded. Returning empty DataFrame.")
            return pd.DataFrame(), newly_failed_symbols
            
        master_df = pd.DataFrame(price_data)
        # Ensure the index is unique before sorting
        if not master_df.index.is_unique:
            master_df = master_df[~master_df.index.duplicated(keep='last')]
            logger.warning("⚠️ Removed duplicate dates in master DataFrame. Kept last entry.")
        
        master_df.sort_index(inplace=True)
        master_df.ffill(inplace=True)
        master_df.bfill(inplace=True)
        master_df.dropna(axis=1, how='any', inplace=True)
        
        logger.info(f"   Master DataFrame created with {len(master_df.columns)} stocks and {len(master_df)} data points.")
        return master_df, newly_failed_symbols
    ...
    def run_preprocessing(self):
        ...
        # The download function now returns the list of newly failed symbols
        self.master_price_df, newly_failed = self.download_all_price_data(universe, max_years, batch_size=50)
        
        # Create the snapshot file only if it doesn't exist
        self._update_failed_symbols_snapshot(newly_failed)

        if self.master_price_df.empty:
            logger.error("❌ Master price DataFrame is empty after downloading. Aborting metric calculation.")
            return

        # Download and merge benchmark data
        benchmark_series = self._download_benchmark_data(max_years)
        if benchmark_series is not None:
            # Merge benchmark data; use 'outer' to keep all dates from both frames
        ...
        # --- Save the Master Price DataFrame ---
        try:
            price_data_path = self.cache_dir / 'master_price_data.feather'
            # Feather format requires a standard index, so we reset it before saving.
            self.master_price_df.reset_index().rename(columns={'index': 'date'}).to_feather(price_data_path)
            logger.info(f"💾 Successfully saved raw price data to: {price_data_path}")
        except Exception as e:
            logger.error(f"❌ Failed to save raw price data: {e}")

        # Exclude benchmark from metric calculation
